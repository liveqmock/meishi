...
;(function($) {
	...
	$.fn.baidumap = function(options) {
		...
		var point = null;
		if(opts.x && opts.y){
			point = new BMap.Point(opts.x, opts.y); // 创建中心点坐标
		}else{
		...
		if (opts.navigationControl) {
			map.addControl(opts.navigationControl);
		}
	 ...
	 * @param address 详细地址
	 */
	$.fn.baidumap.search = function(city, address, listener) {
		var map = $.fn.baidumap.map;
		map.clearOverlays();
		// 创建地址解析器实例
...
//				resultMarker.setAnimation(BMAP_ANIMATION_BOUNCE);
				...
				resultMarker.addEventListener("click", listener);
			}
		}, city);
	};
	...
	/**
	 * 添加右键菜单
	 * @param menuOptions 菜单配置项，值为对象数组，每个对象有text,callback两个参数，text值为‘-’时表示菜单项的分隔符<br/>
	...
	$.fn.baidumap.addContextMenu = function(menuOptions) {
		...
		for ( var i = 0; i < menuOptions.length; i++) {
			if (menuOptions[i].text == "-") {
				...
				contextMenu.addItem(new BMap.MenuItem(menuOptions[i].text, menuOptions[i].callback));
			}
		}
	...
	/**
	 * 新建一个图标
	 * @param options  图标属性 exp .{url:图标路径,width:宽度,height:高度}
	 * @returns BMap.Icon
	 */
	...
	/**
	 * 新建一个文本标注
	 * @param options 标注的属性 exp. {title:标题,offsetX:横向偏移量,offsetY:纵向偏移量,style:css样式}
	 * @returns BMap.Label
	 */
	...
	/**
	 * 新建一个图形标记
	 * @param options 标记的属性 exp.{point:位置 BMap.Point值,icon: 图标BMap.Icon值,label:图标的文本说明}
	 * @returns BMap.Marker
	 */
